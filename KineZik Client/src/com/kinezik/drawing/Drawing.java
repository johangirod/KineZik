...
public class Drawing {
   ...
   //////////////////////
	...
	/** Retourne un indicateur sur le nombre de pics dans le dessin */
	public float getDesc1(){
		return 2*(getBarraquandFormula(2*getNbPeak(), 0)-(float)0.5);
	}
...
//	}
	...
	public float getDesc3(){
		float length = getAvgLineLength()*getLineCount();
		float nPlus = Math.max(length - 800, 0)/200;
		float nMoins = Math.abs(Math.min(length - 800, 0))/200;
		return getBarraquandFormula(nPlus, nMoins);
	}
