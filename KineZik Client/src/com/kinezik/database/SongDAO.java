...
public class SongDAO {
	...
	public void addSong(LocalSong song){
		...
		val.put("UriAlbum", song.getUriAlbum().toString());
		val.put("Genre", song.getGenre());
		val.put("idServer", song.getId());
		Log.d("DEBUG", "Au moment de l'insertion, song.getGenre vaut : " + song.getGenre());
	...
	public ArrayList<LocalSong> getSongs(){
		...
		do{
			...
			curSong.setUriAlbum(Uri.parse(cur.getString(5)));
			curSong.setGenre(cur.getString(6));
			curSong.setId(cur.getLong(7));
			Log.d("DEBUG","Le genre à la sortie de la bd est : "+ curSong.getGenre());
